...
# Output the capital letters of ALL your choices within <answer></answer> tags, e.g., <answer>BCD</answer>.
    ...
    for _ in range(max_tries):
        try:
            ...
            for idx, v in enumerate(videos):
                ...
                video_frames = process_video(path, frames_per_video, length, encode=True)[0]
...
def main():
    ...
    def update(x):
        ...
        if x is not None:
            try:
                input_pair, answer = x
                correct = answer == "".join(input_pair["answer"])
    ...
    print(f"The performance of {model} on task BU is {num_correct / len(results)}")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--QA_path', type=str, help='QA file path.', default='QA/BU.json')
