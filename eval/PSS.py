...
# Provide you {N} shuffled segments of a cooking video, what's the correct order of these segments?
# Watch the segments carefully, and think about the question based on the relationship between these segments.
# Output the correct segment number sequence separated by "->" within <answer></answer> tags, e.g., "2->3->1->4".
...
def evaluate(pair, max_tries=3):
    for _ in range(max_tries):
        try:
            ...
            intervals = [v for k, v in pair["segments"].items()]
            interval_frames = process_video(path, total_frames, intervals, length, encode=True)[0]
            frames = []
            frames_str = ""
            for idx, frame_list in enumerate(interval_frames):
                frame_str = "".join(f"<frame>" for _ in range(len(frame_list)))
                frames_str += f"Segment {idx + 1}:\n"
                frames_str += frame_str
                frames.extend(frame_list)

            prompt = INFERENCE.format(N=len(interval_frames), FRAMES=frames_str)
...
def main():
    ...
    print(f"The performance of {model} on task PSS is {num_correct / len(results)}")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--QA_path', type=str, help='QA file path.', default='QA/PSS.json')
